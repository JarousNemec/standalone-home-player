...
async function loadHome() {
    ...
    try {
        ...
        if (!sections.length) {
            box.innerHTML = '<div class="empty">Nic tu není. Přihlášen? (viz README)</div>';
            return;
        }
        box.innerHTML = "";
...
async function loadPlaylists() {
    ...
    try {
        ...
        if (!pls.length) {
            grid.innerHTML = '<div class="empty">Žádné playlisty. Přihlášen? (viz README)</div>';
            return;
        }
        grid.innerHTML = "";
        for (const p of pls) {
...
async function openDetail(title, fetcher, rowPlay, playAll) {
    ...
    try {
        ...
        pa.className = "play-all";
        pa.textContent = "▶ Přehrát vše";
        pa.onclick = playAll;
        actions.appendChild(pa);
        const list = document.createElement("div");
        list.className = "list";
        tracks.forEach((t, i) => list.appendChild(makeRow(t, () => (detailPlayRow || (() => {
        }))(i))));
...
async function loadLiked() {
    ...
    try {
        ...
        if (!tracks.length) {
            box.innerHTML = '<div class="empty">Žádné oblíbené. Přihlášen? (viz README)</div>';
            return;
        }
        box.innerHTML = "";
...
function renderQueue() {
    ...
    if (!q.length) {
        box.innerHTML = '<div class="empty">Fronta je prázdná.</div>';
        return;
    }
    box.innerHTML = "";
    q.forEach((t, i) => {
...
function connectWS() {
    ...
    ws.onmessage = (ev) => {
        ...
        if (msg.type === "state") applyState(msg);
            ...
            lastState.position = msg.position;
            lastState.duration = msg.duration;
            updateProgress(msg.position, msg.duration);
        }
    };
}
